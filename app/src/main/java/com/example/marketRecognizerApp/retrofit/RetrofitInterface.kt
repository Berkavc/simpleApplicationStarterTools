package com.example.marketRecognizerApp.retrofit
...
interface RetrofitInterface {
 ...
 /*   @GET(Constants.DUMMY)
    fun getDummyList(): Call<List<RoomModels.DummyListModel?>>*/
    ...
    @GET("search?")
    fun getJiraAttachmentList(
        @Query("project") projectKey: String,
        @Query("fields") attachmentTitle: String
    ): Call<JsonObject>
    ...
    @Multipart
    @POST("attachments")
    fun setBasecampAttachments(
        @Part file: MultipartBody.Part, @Header("Content-Length") contentLength: Long, @Query(
            "name"
        ) name: String, @Query("access_token") accessToken: String
    ): Call<JsonObject>

    @POST("uploads")
    fun addBaseAttachments(
        @Body jsonObject: JsonObject,
        @Query("access_token") accessToken: String
    ): Call<JsonObject>

    @POST("todolists")
    fun createBasecampTodo(
        @Body jsonObject: JsonObject,
        @Query("access_token") accessToken: String
    ): Call<JsonObject>
    ...
    @Multipart
    @POST("files")
    fun sendClubhouseAttachments(
        @Query("token") token: String,
        @Part file: MultipartBody.Part
    ): Call<JsonArray>

    @PUT("stories/{id}")
    fun setClubhouseStory(
        @Path("id") id: String,
        @Query("token") token: String,
        @Query("description") description: String
    ): Call<JsonObject>
    ...
    /**Slack Methods**/
    @GET("oauth.v2.access?")
    fun getSlackAccessToken(
        @Query("code") code: String, @Query("client_id") clientId: String, @Query(
            "client_secret"
        ) clientSecret: String
    ): Call<JsonObject>

    @GET("channels.list")
    fun getSlackChannels(): Call<JsonObject>

    @GET("users.list")
    fun getSlackUsers(): Call<JsonObject>

    @POST("chat.postMessage")
    fun createSlackMessage(@Body jsonObject: JsonObject): Call<JsonObject>

    @Multipart
    @POST("files.upload")
    fun setSlackAttachments(
        @Part file: MultipartBody.Part,
