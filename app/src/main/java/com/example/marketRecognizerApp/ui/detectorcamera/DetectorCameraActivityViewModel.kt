...
@HiltViewModel
class DetectorCameraActivityViewModel @Inject constructor(
    @ApplicationContext private val context: Context,
    @RoomDbBuilder private val db: AppDatabase,
    @RetrofitClientBuilder private val retrofitInterFace: RetrofitInterface
) : BaseViewModel() {

    var isCloseClicked = CustomMutableSingleLiveData<Boolean>().default(false)

    var isCaptureClicked = CustomMutableSingleLiveData<Boolean>().default(false)

    fun closeClicked() {
        isCloseClicked.postValue(true)
    }

    fun captureClicked() {
        isCaptureClicked.postValue(true)
    }
